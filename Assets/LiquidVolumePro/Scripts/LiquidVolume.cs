...
namespace LiquidVolumeFX {
    ...
    public partial class LiquidVolume : MonoBehaviour {
        ...
        [SerializeField]
        float _msaaFlaskThicknessCorrection;

        public float msaaFlaskThicknessCorrection {
            get { return _msaaFlaskThicknessCorrection; }
            set {
                if (_msaaFlaskThicknessCorrection != value) {
                    _msaaFlaskThicknessCorrection = value;
                    UpdateMaterialProperties();
                }
            }
        }
        ...
        float prevThickness, prevMSAAThicknessCorrection;

        // Spill point debug
        GameObject spillPointGizmo;
        static readonly string[] defaultContainerNames = {
            "GLASS",
            "CONTAINER",
            "BOTTLE",
            "POTION",
        ...
        void UpdateDirectionalLight() {
            ...
            if (_directionalLight != null) {
                ...
                if (_useLightDirection) {
                    Vector3 lightDir = -_directionalLight.transform.forward;
                    float diff = Mathf.Clamp01(lightDir.y * 2f);
                    lightColor.r *= diff;
                    lightColor.g *= diff;
                    lightColor.b *= diff;
                }
        ...
        void RefreshMaterialProperties() {
            ...
            if (_topology == TOPOLOGY.Irregular && prevThickness != _flaskThickness || prevMSAAThicknessCorrection != _msaaFlaskThicknessCorrection) {
                prevThickness = _flaskThickness;
                prevMSAAThicknessCorrection = _msaaFlaskThicknessCorrection;
                CleanupBackFacesBuffer();
            }
        ...
        /// <summary>
        /// Returns the approximate volume of liquid under certain fill level (fast algorithm)
        /// </summary>
        /// <param name="level01">Fill level (0-1)</param>
        /// <param name="yExtent">y-extent of the volume (ie. renderer.bounds.extents.y)</param>
        ...
        /// Returns the approximate volume of a mesh (fast method)
        /// </summary>
        /// <param name="level">Fill level (0-1)</param>
        public float GetMeshVolumeUnderLevelWSFast(float level) {

            if (mr == null || rotatedVertices == null || verticesIndices == null) return 0;
        ...
        static MaterialPropertyBlock bbPropertyBlock;
        ...
        internal static void SetupBackFacesBuffer(Renderer renderer) {
            ...
            if (bbPropertyBlock == null) {
                bbPropertyBlock = new MaterialPropertyBlock();
            }
            ...
            bbRenderers.ForEach((Renderer obj) => {
                if (obj != null && obj.gameObject.activeSelf) {
                    ...
                    if (liquidVolume != null) {
                        bbPropertyBlock.SetFloat(ShaderParams.FlaskThickness, 1.0f - liquidVolume.flaskThickness + liquidVolume.msaaFlaskThicknessCorrection);
                    } else {
                        bbPropertyBlock.SetFloat(ShaderParams.FlaskThickness, 1.0f);
                    }
                    obj.SetPropertyBlock(bbPropertyBlock);
        ...
        static MaterialPropertyBlock fbPropertyBlock;
        ...
        internal static void SetupFrontFacesBuffer(Renderer renderer) {
            ...
            if (fbPropertyBlock == null) {
                fbPropertyBlock = new MaterialPropertyBlock();
            }
            fbRenderers.ForEach((Renderer obj) => {
                if (obj != null && obj.gameObject.activeSelf) {
                    LiquidVolume liquidVolume = obj.GetComponent<LiquidVolume>();
                    if (liquidVolume != null) {
                        fbPropertyBlock.SetFloat(ShaderParams.FlaskThickness, 1.0f - liquidVolume.flaskThickness + liquidVolume.msaaFlaskThicknessCorrection);
                    } else {
                        fbPropertyBlock.SetFloat(ShaderParams.FlaskThickness, 1.0f);
                    }
                    obj.SetPropertyBlock(fbPropertyBlock);
