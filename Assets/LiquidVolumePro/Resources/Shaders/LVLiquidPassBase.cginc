 ...
 // Open GL 2.0 / WebGL support
	...
	float3 _CustomWorldSpaceCameraPos;
		...
		half4 LightingWrappedSpecular (SurfaceOutput s, half3 lightDir, half3 viewDir, half atten) {
    	    ...
    	    float spec = pow (nh, _GlossinessInternal.x);
	        half4 c;

       	    // apply light scattering
       	    #if defined(LIQUID_VOLUME_SCATTERING)
       	    diff += pow( max( dot( viewDir, -lightDir), 0.0 ), _GlossinessInternal.y) * _GlossinessInternal.z;
       	    #endif
		...
		fixed4 raymarch(float4 vertex, float3 rd, float t0, float t1); // forward declaration
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			float t0, t1;
			float3 camRelPos = IN.realPos - wsCameraPos;
			float dist = length(camRelPos);
			float3 rd = camRelPos / dist;
	...
	*/
					float vz = LinearEyeDepth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, uv));
					float3 viewPlane = camRelPos / dot(camRelPos, unity_WorldToCamera._m20_m21_m22);
					float3 wpos = viewPlane * vz + wsCameraPos;
